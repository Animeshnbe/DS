...
public:
    ...
    virtual vector<TreePtr> & getPointers() {}; 
    virtual map<Key, RecordPtr> & getMap() {};
    virtual void set_next_leaf(TreePtr tp) {};

    //key functions
    virtual TreePtr insert_key(const Key& key, const RecordPtr& record_ptr, TreePtr parent) {return NULL_PTR;}
    virtual void delete_key(const Key& key, TreePtr parent) {}
