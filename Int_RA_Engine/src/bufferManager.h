...
class BufferManager{
    ...
    Page insertIntoPool(string tableName, int pageIndex, bool isaTable);

    public:
    
    BufferManager();
    void deleteFromPool(string name);
    Page getPage(string tableName, int pageIndex, bool isaTable=true);
    ...
    void writePage(string tableName, int pageIndex, vector<int> &rows, int rowCount, int colCount);
};