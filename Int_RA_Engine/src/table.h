...
public:
    ...
    vector<pair<int,bool>> cols;
    unordered_map<int,vector<vector<int>>> spages; // always 9 pages
    ...
    void tablesort(string newname);
    void join(Table& ftable, Table& stable,string binop);
    void group(Table* table, int ind, pair<string,string> agg, string binop, int val, pair<string,string> ret);
    void checkwrite(vector<vector<int>>& mat, bool dowrite=false);
    void unload();
